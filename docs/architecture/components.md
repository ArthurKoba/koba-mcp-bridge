...
| `common` | — | shared runtime/config, control-plane client, HTTP transport and Git-domain primitives |
| `control_plane` | `control-plane` | provider account registry, encrypted credentials, admin UI and invocation telemetry |
...
`control_plane.domain` and `control_plane.application` are independent of FastAPI,
...
`account_id` over the authenticated private control-plane API. Provider credentials are
...
`modules.<name>.runtime:app` or `control_plane.runtime:app`. Only `gateway` is public.
