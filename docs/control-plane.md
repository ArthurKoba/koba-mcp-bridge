...
The control plane is the private account, credential and lightweight telemetry service for
MCP Bridge. It is part of the same source repository and follows the same application
architecture, but runs as its own container so provider runtimes do not share a SQLite
connection or encryption key.
...
Production storage is one persistent SQLite database under `/control-plane`. SQLAlchemy
is the persistence adapter and Alembic is the schema migration authority. The database is
owned only by the `control-plane` process.
...
`reviewer`. `external_id` stores the GitHub App ID and the encrypted credential stores the
App private key.

GitLab accounts use the `general` role and one of `private_token`, `bearer` or `job_token`.
`base_url` is stored per account, so gitlab.com and self-hosted GitLab instances use the
same contract. TLS verification and an optional CA certificate PEM are also per account.

## Internal API

Provider runtimes authenticate with `CONTROL_PLANE_SERVICE_TOKEN`. The internal API lets
...
Starlette Admin is mounted at `/admin` inside the private control-plane runtime and reverse-proxied by the public gateway at the same `/admin` path. Admin login is
...
The old Infisical provider/account registry is removed. Configure only the control-plane
bootstrap and gateway OAuth secrets in the deployment environment:

```text
CONTROL_PLANE_ENCRYPTION_KEY=<Fernet key>
CONTROL_PLANE_SERVICE_TOKEN=<random internal token>
CONTROL_PLANE_ADMIN_USERNAME=admin
CONTROL_PLANE_ADMIN_PASSWORD=<strong password>
CONTROL_PLANE_SESSION_SECRET=<random session secret>
CONTROL_PLANE_SESSION_HTTPS_ONLY=true
